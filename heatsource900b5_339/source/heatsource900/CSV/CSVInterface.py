...
class CSVInterface(object):
    ...
    def SetupInputFiles(self, inputdir, control_file):
        ...
        climatefile = pd.DataFrame(index=timelist, columns=['GlobalRadiation', 'WindSpeed','RelativeHumidity','AirTemp']*int((IniParams["contsites"]/len(climatefiles))))
        if IniParams["inflowsites"] > 0:
    ...
    def GetContinuousData(self):
        ...
        for time in timelist:
            ...
            for glorad, wind, humid, air in line:
                ...
                if glorad is None: glorad = 0.0
                if wind is None: wind = 0.0
                #if cloud < 0 or cloud > 1:
                if glorad < 0 or glorad > 1500:
                    ...
                    else: raise Exception("GlobalRadiation (value of '%s' in Continuous Data) must be greater than zero and less than 1500." % `glorad`)
                #humid = float(humid) #TODO: Gibt es eine elegantere Loesung?
                if humid < 0 or humid is None or humid > 1:
                    if self.run_type == 1: # Alright in shade-a-lator
                        humid = 0.0
                    else: raise Exception("Humidity value of '%s' in Continuous Data) must be greater than zero and less than one. "
                                          "types glorad, wind, humid, air = " % `humid`, type(glorad), type(wind), type(humid), type(air))

                #air = float(air) #TODO: Gibt es eine elegantere Loesung?
                ...
                node.ContData[time] = glorad, wind, humid, air
